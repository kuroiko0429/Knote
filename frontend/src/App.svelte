<script lang="ts">
  ...
  import { EditorView, highlightSpecialChars, drawSelection, dropCursor, highlightActiveLine, keymap, WidgetType } from '@codemirror/view'
...
<main bind:this={mainEl} class:compact={compactMode} style="grid-template-columns: {showSidebar ? sidebarWidth : 0}px {compactMode ? '1fr 0' : `min(${editorWidth}px, calc(100% - ${showSidebar ? sidebarWidth : 0}px - 180px)) minmax(180px, 1fr)`}">
  ...
  {#if showSettings}
    <SettingsPanel
      ...
      on:themeSelect={async (e) => { await selectTheme(e.detail); themeList = await ListThemes() }}
      on:vaultChange={() => changeVault()}
      on:snippetsChange={(e) => { snippets = e.detail }}
      on:toggleTheme={() => toggleTheme()}
      on:toggleVim={() => toggleVim()}
      on:templatesChange={(e) => { templateList = e.detail }}
      on:dailyNoteChange={(e) => { dailyNoteFolder = e.detail.folder; dailyNoteTemplate = e.detail.template }}
    />
  {/if}
